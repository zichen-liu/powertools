...
#' @param sigma The estimated standard deviation; defaults to 1.
...
#' @examples
#' # Example 3.2
#' pss.t.test.1samp(n = 36, delta = 4.9 - 5.7, sigma = 2, one.or.two.sided = "one")
#' # Example 3.3
#' pss.t.test.1samp(n = 36, delta = 6.3 - 5.7, sigma = 2, one.or.two.sided = "one")
#' # Example 3.5
#' pss.t.test.1samp(n = 36, delta = 4.9 - 5.7, sigma = 2, one.or.two.sided = "two")
#' # Example 3.6
#' pss.t.test.1samp(delta = 0.6, sigma = 1, power = 0.8, one.or.two.sided = "one")

pss.t.test.1samp <- function (n = NULL, delta = NULL, sigma = 1,
                              alpha = 0.05, power = NULL,
                              one.or.two.sided = c("two", "one"), strict = TRUE) {

  # Check if the arguments are specified correctly
  if (sum(sapply(list(n, delta, sigma, power, alpha), is.null)) != 1)
    stop("exactly one of n, delta, sigma, alpha, and power must be NULL")
