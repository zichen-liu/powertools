...
#' # explores range itself from N = 30 to 150
...
anova3_ss_wrapper <- function(means, sds, alphalevel, powerlevel, groupweights,
                              Nmin = NULL, Nmax = NULL, Nchange = NULL){
  ...
  # update the n's to be the last n's that weren't enough
    n1_last <- last_calc[1]; n2_last <- last_calc[2]; n3_last <- last_calc[3];
  for(i in 1:Nchange){
